...
lengthGT4 ::
  List a
  -> Bool
lengthGT4 = flip (??) False . mapOptional (const True) . find (== 5) . runningLength 1
  where runningLength _ Nil      = Nil
        runningLength n (_ :. t) = n :. runningLength (n+1) t
...
notReverse ::
  List a
  -> List a
-- TODO: impossible due to parametricity and tests
notReverse = reverse
